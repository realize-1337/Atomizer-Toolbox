...
def edit():
    ...
    scripts = os.path.dirname(python_interpreter_path)
    venv = os.path.dirname(scripts)

    matlabengine = os.path.join(venv, 'Lib', 'site-packages' ,'matlab', '__init__.py')
    arch = os.path.join(os.path.dirname(matlabengine), 'engine', '_arch.txt')
...
'''
    ...
    with open(matlabengine, 'w') as file:
        file.write(EDIT)
